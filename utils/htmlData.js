var HtmlData= {
    image:"data:image/jpeg;base64,",
    firstName: 'James',
    lastName: 'Bond',
    address: "440-A Railway Gas Colony 99 Railway Road Lahore",
    email:"m.rehmangames@gmail.com",
    phone:"03344293021",
    country: "Pakistan",
    gender: "male",
    jobTitle:"Full Stack Developer",
    perfessionalSummery: "A resume summary, also known as a professional summary or summary statement, is a one to two sentence description at the start of your resume that describes you, your level of experience and your key skills. Using a resume summary allows you to showcase your strongest assets upfront..",
    experienceWith: ["React" , "Angular" , "Node" , "Type-Script" , "Vue" ,"Html/Css"],
    eductionDetail: [{
        title: "Matric",
        inst_name:"Lahore garrison school",
        from:"2009",
        to:"2011",
        detail:"A resume summary, also known as a professional summary or summary statement, is a one to two sentence description at the start of your resume that describes you, your level of experience and your key skills. Using a resume summary allows you to showcase your strongest assets upfront.."
    },
    {
        title: "Inter",
        inst_name:"GC College Lahore",
        from:"2012 ",
        to:"2014",
        detail:"A resume summary, also known as a professional summary or summary statement, is a one to two sentence description at the start of your resume that describes you, your level of experience and your key skills. Using a resume summary allows you to showcase your strongest assets upfront.."
    },
    {
        title: "BS(Hons)IT",
        inst_name:"Fast UNIVERSITY ",
        from:"2014",
        to:"2018",
        detail:"A resume summary, also known as a professional summary or summary statement, is a one to two sentence description at the start of your resume that describes you, your level of experience and your key skills. Using a resume summary allows you to showcase your strongest assets upfront.."
    },
    {
        title: "Master",
        inst_name:"Faster",
        from:"2019",
        to:"2020",
        detail:"A resume summary, also known as a professional summary or summary statement, is a one to two sentence description at the start of your resume that describes you, your level of experience and your key skills. Using a resume summary allows you to showcase your strongest assets upfront.."
    }
   ],
    jobDetail: [{
        title: "Python Odod System Manager",
        inst_name:"Park Rendel",
        from:"2015",
        to:"2016",
        detail:"Manage All lead on system also develope small module on system"
    },
    {
        title: "Web Developer FrontEnd Design",
        inst_name:"World soft",
        from:"2016",
        to:"2016",
        detail:"Covert PSD in to Web Design"
    },
    {
        title: "MEAN STACK",
        inst_name:"Novatore Solution",
        from:"2018",
        to:"2019",
        detail:"Working on node mongodb angular manage stand alone projects in system"
    },
    {
        title: "MERN & MARN",
        inst_name:"soft tech",
        from:"2019",
        to:"2020",
        detail:"Manage All lead on system also develope small module on system"
    }
    ],
    projectDetail: [{
        title: "glimer",
        from: "2017",
        to: "2018",
        detail: "web that belpng to dentail docoters and many more web that belpng to dentail docoters and many more"
      },
      {
        title: "glimer 2",
        from: "2017",
        to: "2018",
        detail: "web that belpng to dentail docoters and many mor web that belpng to dentail docoters and many moree"
      }
      ,
      {
        title: "glimer 3",
        from: "2017",
        to: "2018",
        detail: " web that belpng to dentail docoters and many more  web that belpng to dentail docoters and many more  web that belpng to dentail docoters and many more"
      }
    ],
    hobbies : ["Garding","Cricket","Reading"],
    languages : ["English", "Urdu"]
}

module.exports = {HtmlData}